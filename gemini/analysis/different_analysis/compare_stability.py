...
if __name__ == '__main__':
    ...
    # 1. 动态拟合模型参数
    params = {}
    print("\n[Step 1] Fitting decision model for Gemini (Eng)...")
    beta0_en, lambda_en = get_logit_params('gemini_en_fitting')
    params['eng'] = {'beta0': beta0_en, 'lambda': lambda_en}
    
    print("\n[Step 2] Fitting decision model for gemini (Chinese)...")
    beta0_zh, lambda_zh = get_logit_params('gemini_zh_fitting')
    ...
    print("\n[Step 4] Generating matchings for Classic Gale-Shapley (based on 'objective' and 'gemini_score')...")
    objective_matchings = []
    ...
    if params['eng']['beta0'] is not None:
        results['gemini (Eng) Actual Run'] = calculate_expected_blocking_pairs(gemini_en_matchings_run, source_scores_total, params['eng']['beta0'], params['eng']['lambda'], df_source, "gemini (Eng) Run")
    if params['zh']['beta0'] is not None:
        results['gemini (Chinese) Actual Run'] = calculate_expected_blocking_pairs(gemini_zh_matchings_run, source_scores_total, params['zh']['beta0'], params['zh']['lambda'], df_source, "gemini (Chi) Run")
    ...
    # 评估基于gemini分数的经典GS匹配
    if params['eng']['beta0'] is not None:
        results['gpt-score GS (judged by Eng AI)'] = calculate_expected_blocking_pairs(gemini_guided_matchings, source_scores_total, params['eng']['beta0'], params['eng']['lambda'], df_source, "gemini-guided (Eng Eval)")
    if params['zh']['beta0'] is not None:
        results['gpt-score GS (judged by Chi AI)'] = calculate_expected_blocking_pairs(gemini_guided_matchings, source_scores_total, params['zh']['beta0'], params['zh']['lambda'], df_source, "gemini-guided (Chi Eval)")
