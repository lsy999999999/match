# 文件名: gale_shapley_counterfactual.py (最终版)

# --------------------------------------------------------------------------
#  导入部分 (与你之前的脚本完全一致)
# --------------------------------------------------------------------------
from read_file_random import read_file, read_file_attr
from ask_ds_eng import (
...
def gale_shapley(men_preferences, women_preferences, men_attr, women_attr, output_idx):
    ...
    # --- 文件保存 ---
    output_path = "/home/lsy/match/deepseek/0703_ds_eng"
    os.makedirs(output_path, exist_ok=True)
    csv_filename = os.path.join(output_path, f"0703_ds_eng_group{output_idx}.csv")
    json_filename = os.path.join(output_path, f"0703_ds_eng_group{output_idx}.json")
...
if __name__ == '__main__':
  # 定义输入组号和输出组号的起始点
  input_start_group = 21
  output_start_group = 41
  
  num_simulations = 1 # 我们要进行21次模拟
