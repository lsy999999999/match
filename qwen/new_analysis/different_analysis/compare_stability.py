...
if __name__ == '__main__':
    ...
    beta0_en, lambda_en, s0_en = get_unified_model_params('qwen_en_fitting')
    beta0_zh, lambda_zh, s0_zh = get_unified_model_params('qwen_zh_fitting')
    ...
    gpt4_en_matchings_run = load_matchings_from_json('qwen_en_fitting')
    gpt4_zh_matchings_run = load_matchings_from_json('qwen_zh_fitting')
    ...
    if all(p is not None for p in params['eng'].values()):
        results['AI Run (judged by Eng AI)'] = calculate_expected_blocking_pairs_unified(gpt4_en_matchings_run, source_scores_total, params['eng']['beta0'], params['eng']['lambda'], params['eng']['s0'], df_source, "AI Run (Eng Eval)")
    if all(p is not None for p in params['zh'].values()):
        results['AI Run (judged by Chi AI)'] = calculate_expected_blocking_pairs_unified(gpt4_zh_matchings_run, source_scores_total, params['zh']['beta0'], params['zh']['lambda'], params['zh']['s0'], df_source, "AI Run (Chi Eval)")
