...
if __name__ == '__main__':
    ...
    # 1. 动态拟合模型参数
    params = {}
    print("\n[Step 1] Fitting decision model for qwen (Eng)...")
    beta0_en, lambda_en = get_logit_params('qwen_en_fitting')
    params['eng'] = {'beta0': beta0_en, 'lambda': lambda_en}
    
    print("\n[Step 2] Fitting decision model for qwen (Chinese)...")
    beta0_zh, lambda_zh = get_logit_params('qwen_zh_fitting')
    ...
    qwen_en_matchings_run = load_matchings_from_json('qwen_en_fitting')
    qwen_zh_matchings_run = load_matchings_from_json('qwen_zh_fitting')
    
    # 3. 为经典Gale-Shapley算法生成匹配结果
    print("\n[Step 4] Generating matchings for Classic Gale-Shapley (based on 'objective' and 'qwen_score')...")
    objective_matchings = []
    qwen_guided_matchings = []
    ...
    num_groups = config['qwen_en_fitting']['num_groups']
    ...
    for i in range(1, num_groups + 1):
        ...
        # b. qwen指导的匹配
        men_prefs_qwen = {m: group_df[group_df['iid'] == m].sort_values(by='gpt_score', ascending=False)['pid'].tolist() for m in men_ids}
        women_prefs_qwen = {w: group_df[group_df['pid'] == w].sort_values(by='gpt_score', ascending=False)['iid'].tolist() for w in women_ids}
        qwen_guided_matchings.append(classic_gale_shapley_matcher(men_prefs_qwen, women_prefs_qwen))
    ...
    if params['eng']['beta0'] is not None:
        results['qwen (Eng) Actual Run'] = calculate_expected_blocking_pairs(qwen_en_matchings_run, source_scores_total, params['eng']['beta0'], params['eng']['lambda'], df_source, "qwen (Eng) Run")
    if params['zh']['beta0'] is not None:
        results['qwen (Chinese) Actual Run'] = calculate_expected_blocking_pairs(qwen_zh_matchings_run, source_scores_total, params['zh']['beta0'], params['zh']['lambda'], df_source, "qwen (Chi) Run")
    ...
    # 评估基于qwen分数的经典GS匹配
    if params['eng']['beta0'] is not None:
        results['qwen-score GS (judged by Eng AI)'] = calculate_expected_blocking_pairs(qwen_guided_matchings, source_scores_total, params['eng']['beta0'], params['eng']['lambda'], df_source, "qwen-guided (Eng Eval)")
    if params['zh']['beta0'] is not None:
        results['qwen-score GS (judged by Chi AI)'] = calculate_expected_blocking_pairs(qwen_guided_matchings, source_scores_total, params['zh']['beta0'], params['zh']['lambda'], df_source, "qwen-guided (Chi Eval)")
