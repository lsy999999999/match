...
if __name__ == '__main__':
    ...
    params_en = get_logit_params_for_both_scenarios('qwen_en_fitting')
    params_zh = get_logit_params_for_both_scenarios('qwen_zh_fitting')
    ...
    print("\n[Step 3] Generating matchings for Classic Gale-Shapley (based on 'objective' and 'qwen_score')...")
    objective_matchings = []
    ...
    if params_en:
        results['AI Run (judged by Eng AI)'] = calculate_expected_blocking_pairs_advanced(qwen_en_matchings_run, source_scores_total, params_en, df_source, "AI Run (Eng Eval)")
    if params_zh:
        results['AI Run (judged by Chi AI)'] = calculate_expected_blocking_pairs_advanced(qwen_zh_matchings_run, source_scores_total, params_zh, df_source, "AI Run (Chi Eval)")
    ...
    if params_en:
        results['GPT-score GS (judged by Eng AI)'] = calculate_expected_blocking_pairs_advanced(qwen_guided_matchings, source_scores_total, params_en, df_source, "GPT-score GS (Eng Eval)")
    if params_zh:
        results['GPT-score GS (judged by Chi AI)'] = calculate_expected_blocking_pairs_advanced(qwen_guided_matchings, source_scores_total, params_zh, df_source, "GPT-score GS (Chi Eval)")
