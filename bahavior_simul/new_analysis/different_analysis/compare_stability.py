...
if __name__ == '__main__':
    ...
    # 1. 动态拟合模型参数
    params = {}
    print("\n[Step 1] Fitting decision model for GPT-4 (Eng)...")
    beta0_en, lambda_en = get_logit_params('gpt4_en_fitting')
    params['eng'] = {'beta0': beta0_en, 'lambda': lambda_en}
    
    print("\n[Step 2] Fitting decision model for GPT-4 (Chinese)...")
    beta0_zh, lambda_zh = get_logit_params('gpt4_zh_fitting')
    ...
    for i in range(1, num_groups + 1):
        ...
        # b. GPT指导的匹配
        men_prefs_gpt = {m: group_df[group_df['iid'] == m].sort_values(by='gpt_score', ascending=False)['pid'].tolist() for m in men_ids}
        women_prefs_gpt = {w: group_df[group_df['pid'] == w].sort_values(by='gpt_score', ascending=False)['iid'].tolist() for w in women_ids}
        gpt_guided_matchings.append(classic_gale_shapley_matcher(men_prefs_gpt, women_prefs_gpt))
    ...
    if params['eng']['beta0'] is not None:
        results['GPT-4 (Eng) Actual Run'] = calculate_expected_blocking_pairs(gpt4_en_matchings_run, source_scores_total, params['eng']['beta0'], params['eng']['lambda'], df_source, "GPT-4 (Eng) Run")
    if params['zh']['beta0'] is not None:
        results['GPT-4 (Chinese) Actual Run'] = calculate_expected_blocking_pairs(gpt4_zh_matchings_run, source_scores_total, params['zh']['beta0'], params['zh']['lambda'], df_source, "GPT-4 (Chi) Run")
    ...
    # 评估基于GPT分数的经典GS匹配
    if params['eng']['beta0'] is not None:
        results['GPT-score GS (judged by Eng AI)'] = calculate_expected_blocking_pairs(gpt_guided_matchings, source_scores_total, params['eng']['beta0'], params['eng']['lambda'], df_source, "GPT-guided (Eng Eval)")
    if params['zh']['beta0'] is not None:
        results['GPT-score GS (judged by Chi AI)'] = calculate_expected_blocking_pairs(gpt_guided_matchings, source_scores_total, params['zh']['beta0'], params['zh']['lambda'], df_source, "GPT-guided (Chi Eval)")
