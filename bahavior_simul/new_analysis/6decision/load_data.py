...
# load_all_group_data_for_model 函数保持不变
...
def load_source_scores_vectorized(path):
    """
    加载原始Excel评分数据，返回一个方便查询每个维度分数的字典。
    """
    try:
        df = pd.read_excel(path)
        ...
        if 'ambitous' in df.columns: df.rename(columns={'ambitous': 'ambition'}, inplace=True)
