...
# 1. 左上：接受概率 vs 分数差异 (Acceptance Probability vs Score Difference)
# 含义：

# X轴：分数差异 |Sa - Sb|（0-7分）
# Y轴：接受概率（0-1）
# 蓝色圆点：实际数据的接受率（按分数差异分组后计算）
# 蓝线：MLE方法拟合的概率曲线 P = e^(-1.412×差异)
# 橙线：MSE方法拟合的概率曲线 P = e^(-3.316×差异)
...
# # 计算每个区间内的平均接受率
# for each bin:
#     接受率 = 该区间内接受的决策数 / 该区间内总决策数
# 解读：

# 分数差异越大，接受概率越低
# λ=3.316（MSE）下降更快，说明对分数差异更敏感

# 2. 右上：分数差异分布 (Distribution of Score Differences by Decision)
# 含义：
...
# 接受的决策主要集中在小分数差异（0-2分）
# 拒绝的决策分数差异更分散（0-4分）
# 重叠区域表明决策不完全由分数差异决定

# 3. 左下：单身决策散点图 (Single Person Decisions)
# 含义：
...
# 4. 右下：换伴侣决策散点图 (Switch Decisions)
# 含义：
...
# 对角线上方：新人分数更高
# 对角线下方：现任分数更高
# 绿点主要在对角线上方，说明倾向于选择分数更高的
# 但也有例外（对角线下方的绿点），说明决策不完全基于分数
