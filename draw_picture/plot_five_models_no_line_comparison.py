# -*- coding: utf-8 -*-
"""
plot_five_models_scatter_subplots.py

功能：
生成 5 个大模型的 "Random vs AI" 对角线散点对比图。
- 布局：2行3列 (前5个为子图，第6个为图例)。
- 内容：每个子图中，X轴为 Random E[BP]，Y轴为 AI E[BP]。每个点代表一个 Group。
- 逻辑：点位于 y=x 对角线下方表示 AI 比 Random 更稳定。
...
OUTPUT_PDF = "/home/lsy/match/draw_picture/pictures/five_models_scatter_subplots.pdf"
# ============================================================
...
def main():
    ...
    for i in range(5):
        ...
        if name in all_data:
            ...
            # 1. 绘制对角线 y=x
            ax.plot([0, axis_limit], [0, axis_limit], 'k--', alpha=0.4, linewidth=1.5, zorder=1)
            
            # 2. 绘制散点 (每个点代表一个 Group)
            # Random 在 X 轴，AI 在 Y 轴
            ax.scatter(df['Ebp_Random'], df['Ebp_AI'], 
                       color=color, s=100, alpha=0.85, edgecolors='w', linewidth=0.5, zorder=3)
            
            # 3. 填充 "AI Better" 区域 (下三角)
            ax.fill_between([0, axis_limit], [0, 0], [0, axis_limit], color=color, alpha=0.05, zorder=0)
            
            # 标题 (放在下方)
            ax.set_title(f"{identifiers[i]} {name}", y=-0.25, fontsize=14, fontweight='bold')
            
            # 坐标轴标签
            # 只有第一列和第二列(如果需要)显示标签，或者为了清晰每个都显示
            # 这里为了布局整洁：
            # 第一列显示 Y 轴标签
            if i % 3 == 0:
                ax.set_ylabel("AI Agent E[BP]", fontsize=12)
            else:
                ax.set_ylabel("")
                
            # 第二行显示 X 轴标签
            if i >= 3: # (d), (e)
                ax.set_xlabel("Random Baseline E[BP]", fontsize=12)
            elif i == 2: # (c) 右上角那个也加上 X 轴标签比较好，或者统一不加
                ax.set_xlabel("") 
            
            # 统一坐标轴范围，强制为正方形比例，方便视觉对比
    ...
    # 构造图例元素
    legend_elements = [
        Line2D([0], [0], marker='o', color='w', markerfacecolor='gray', markersize=12, label='One Group (Social Circle)'),
        Line2D([0], [0], linestyle='--', color='k', alpha=0.6, linewidth=2, label='y=x (No Improvement)'),
    ]
    
    # 添加文字说明
    legend_ax.text(0.5, 0.6, "Interpretation:", ha='center', fontsize=12, fontweight='bold')
    legend_ax.text(0.5, 0.45, "Points below diagonal line\nindicate AI is more stable\nthan Random Baseline.", 
    ...
    print(f"\n[Success] Scatter subplots saved to: {OUTPUT_PDF}")
