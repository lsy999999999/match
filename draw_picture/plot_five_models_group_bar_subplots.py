...
功能：
生成 5 个大模型的 "Random vs AI" 分组对比图 (基于方案1)。
- 布局：2行3列 (前5个为子图，第6个为图例)。
- 内容：每个子图中，X轴为 Group ID (1-21)，Y轴为 E[BP]。
- 逻辑：
    1. 灰色柱子 (Bar): 代表该组的 Random Baseline E[BP]。
    2. 彩色圆点 (Point): 代表该组的 AI E[BP]。
    3. 视觉解读: AI 圆点位于灰色柱子内部（下方），直观展示稳定性提升幅度。
...
OUTPUT_PDF = "/home/lsy/match/draw_picture/pictures/five_models_bar_comparison.pdf"
# ============================================================
...
def main():
    ...
    for i in range(5):
        ...
        if name in all_data:
            ...
            # 1. 绘制 Random Bar (背景柱)
            ax.bar(groups, df['Ebp_Random'], color='gray', alpha=0.35, width=0.6, label='_nolegend_')
            
            # 2. 绘制 AI Point (前景点)
            ax.scatter(groups, df['Ebp_AI'], color=color, s=60, zorder=5, alpha=0.9)
    ...
    # 构造自定义图例
    legend_elements = [
        mpatches.Patch(color='gray', alpha=0.35, label='Random Baseline (Bar)'),
        Line2D([0], [0], marker='o', color='w', markerfacecolor='black', markersize=10, label='AI Agent Performance (Point)'),
    ]
    
    # 添加解释文字
    legend_ax.text(0.5, 0.65, "Interpretation:", ha='center', fontsize=12, fontweight='bold')
    legend_ax.text(0.5, 0.45, "Lower points indicate\nbetter stability compared\nto the random baseline.", 
    ...
    print(f"\n[Success] Bar comparison chart saved to: {OUTPUT_PDF}")
